    ...
    FaceLoginService face, RefreshTokenService refreshTokens) : ControllerBase
{
    ...
    public async Task<ActionResult<LoginResponse>> Refresh([FromBody] RefreshRequest? req = null)
    {
        ...
        var raw = Request.Cookies[WunderkindLC.Server.AuthCookies.RtCookie];
    ...
    private void IssueAuthCookies(string token, string refreshToken, DateTime refreshExpiresUtc)
    {
        ...
        WunderkindLC.Server.AuthCookies.Issue(HttpContext, token, expiresUtc, refreshToken, refreshExpiresUtc);
    }
    ...
    public async Task<IActionResult> Logout([FromBody] RefreshRequest? req = null)
    {
        ...
        WunderkindLC.Server.UploadsGuard.ClearCookie(HttpContext);
        // DUAL-MODE cookie'lari ham o'chiriladi (umumiy kompyuterda keyingi odam kirmasin).
        Response.Cookies.Delete(WunderkindLC.Server.AuthCookies.AtCookie, new CookieOptions { Path = "/" });
        Response.Cookies.Delete(WunderkindLC.Server.AuthCookies.CsrfCookie, new CookieOptions { Path = "/" });
        WunderkindLC.Server.AuthCookies.ClearRefresh(HttpContext);
        return NoContent();
    }
