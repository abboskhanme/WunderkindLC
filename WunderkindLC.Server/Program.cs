...
using WunderkindLC.Server.Controllers;
using WunderkindLC.Server.Cti;
...
if (Directory.Exists(keysDir))
    builder.Services.AddDataProtection()
        ...
        .SetApplicationName("WunderkindLC");
        ...
        options.Events = new JwtBearerEvents
        {
            OnMessageReceived = context =>
            {
                ...
                if (string.IsNullOrEmpty(context.Token)
                    ...
                    && context.Request.Cookies.TryGetValue(WunderkindLC.Server.AuthCookies.AtCookie, out var atCookie)
...
builder.Services.AddHostedService<WunderkindLC.Application.Services.WorkTaskReminderService>();
// Kunlik markaz AI tahlili (ertalab ~8:00 Toshkent; Gemini kaliti + AiDailyAnalysisEnabled bo'lsa).
builder.Services.AddHostedService<WunderkindLC.Application.Services.CenterAiSchedulerService>();
// KPI: oy BOSHIDAGI surat (faol o'quvchilar soni, qarz qoldig'i) — 1-kuni yarim tundan keyin.
builder.Services.AddHostedService<WunderkindLC.Application.Services.Kpi.KpiSnapshotService>();
...
// KARYERA (Wunderkind Career) — ALOHIDA bot (.env: CAREER_BOT_TOKEN) + `/vakansiya` Mini App.
...
builder.Services.AddHttpClient<WunderkindLC.Application.Services.InstagramApi>();
...
builder.Services.AddSingleton<WunderkindLC.Application.Services.InstagramPipeline>();
...
builder.Services.AddScoped<WunderkindLC.Application.Services.InstagramFaqSync>();
...
builder.Services.AddHostedService<WunderkindLC.Application.Services.InstagramWorkerService>();
...
builder.Services.AddHttpClient<WunderkindLC.Application.Services.MetaAdsApi>();
...
builder.Services.AddScoped<WunderkindLC.Application.Services.MetaLeadgenService>();
...
builder.Services.AddHttpClient<WunderkindLC.Application.Services.MetaInsightsApi>();
builder.Services.AddScoped<WunderkindLC.Application.Services.MetaInsightsService>();
builder.Services.AddHttpClient<WunderkindLC.Application.Services.MetaCapiApi>();
builder.Services.AddScoped<WunderkindLC.Application.Services.MetaCapiService>();
builder.Services.AddHttpClient<WunderkindLC.Application.Services.InstagramPublishApi>();
builder.Services.AddScoped<WunderkindLC.Application.Services.InstagramPublishService>();
...
builder.Services.AddScoped<WunderkindLC.Application.Services.IgEmbeddingService>();
...
builder.Services.AddScoped<WunderkindLC.Application.Services.MarketingMediaCleanup>();
...
builder.Services.AddScoped<WunderkindLC.Application.Services.AuditService>();
// Bog'lanish navbatiga qo'shish — admin va O'QITUVCHI controllerlari uchun yagona manba.
builder.Services.AddScoped<WunderkindLC.Application.Services.ContactQueueService>();
...
builder.Services.AddScoped<WunderkindLC.Application.Services.FaceLoginService>();
...
builder.Services.AddSingleton(WunderkindLC.Application.Services.FaceVault.FromSecrets());
// Ilova haqiqiyligi (Google Play Integrity) — OAuth tokenini keshlaydi → Singleton.
builder.Services.AddSingleton<WunderkindLC.Application.Services.AppAttestation>();

// Shartnoma andozasini (Word) to'ldirish xizmati
builder.Services.AddScoped<WunderkindLC.Application.Services.ContractService>();

// Sertifikat tizimi (HTML yaratish, hash, tekshirish)
builder.Services.AddScoped<WunderkindLC.Application.Services.CertificateService>();
...
builder.Services.AddSingleton<WunderkindLC.Application.Services.DocxToPdfConverter>();
builder.Services.AddScoped<WunderkindLC.Application.Services.TestCertificateService>();
...
builder.Services.AddSingleton<WunderkindLC.Application.Services.TestCertificateJobs>();
...
builder.Services.AddSingleton<WunderkindLC.Server.UploadsGuard>();

// Turniket/FaceID integratsiyasi — o'qituvchilar davomatini avtomatik yuklash
builder.Services.AddScoped<WunderkindLC.Application.Services.TurnstileService>();

// Xona vaqt konflikti aniqlash (guruh yaratish/tahrirlashda warning)
builder.Services.AddScoped<WunderkindLC.Application.Services.RoomConflictService>();

// Xona bandlik va samaradorlik metrikalari
builder.Services.AddScoped<WunderkindLC.Application.Services.RoomUtilizationService>();
builder.Services.AddScoped<WunderkindLC.Application.Services.ScheduleService>();

// Kamera (videokuzatuv) media-shlyuzi (MediaMTX) bilan ishlash
builder.Services.AddHttpClient<WunderkindLC.Application.Services.CameraGateway>();
...
builder.Services.AddSingleton<WunderkindLC.Application.Services.NvrArchiveService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    //
    // ⚠️ DEFAULT qiymatlar ATAYIN BO'SH (''). Ilgari bu yerda saytning namunaviy havolasi,
    // e-pochtasi va telefon raqami kabi "namuna" qiymatlar turardi va natijada YANGI
...
try
{
    var (kpiRules, kpiTemplates, kpiItems) =
        await WunderkindLC.Application.Services.Kpi.KpiSeedService.EnsureAsync(db, "Tizim");
...
var faceFolders = new[]
{
    Path.Combine(uploadsDir, WunderkindLC.Application.Services.FaceStorage.FolderName),
    Path.Combine(webRootForPrivate, "uploads", WunderkindLC.Application.Services.FaceStorage.FolderName),
};
...
if (!WunderkindLC.Application.Services.AppSecrets.FaceVectorKeyConfigured)
...
var marketingPublicDir = Path.Combine(
    uploadsDir, WunderkindLC.Application.Services.MarketingPublicMedia.FolderName);
...
var marketingPublicTypes = new Microsoft.AspNetCore.StaticFiles.FileExtensionContentTypeProvider(
    new Dictionary<string, string>(
        WunderkindLC.Application.Services.MarketingPublicMedia.ContentTypes,
        StringComparer.OrdinalIgnoreCase));

app.UseStaticFiles(new StaticFileOptions
{
    FileProvider = new PhysicalFileProvider(marketingPublicDir),
    RequestPath = WunderkindLC.Application.Services.MarketingPublicMedia.RequestPath,
...
var uploadsGuard = app.Services.GetRequiredService<WunderkindLC.Server.UploadsGuard>();
...
IFileProvider Guarded(IFileProvider innerProvider) =>
    new WunderkindLC.Application.Services.PrivateFolderFileProvider(
...
app.UseMiddleware<WunderkindLC.Server.CsrfMiddleware>();
...
// SPA fallback: barcha hostlar (wunderkindedu.uz, lc.wunderkindedu.uz, ...) → React SPA
