...
public class CertificateTests : IDisposable
{
    ...
    private readonly string _root = Path.Combine(
        Path.GetTempPath(), "wunderkind-cert-tests", Guid.NewGuid().ToString("N"));
    ...
    public async Task Generate_OqituvchiBerilmasa_GuruhOqituvchisidanOlinadi()
    {
        ...
        ctx.Classes.Add(new WunderkindLC.Domain.Group { Name = "A1-1", CourseId = course.Id, TeacherId = teacher.Id });
    ...
    public async Task Generate_TekshirishUrli_SertifikatIdSiBilan()
    {
        ...
        Assert.Equal($"<x>https://lc.wunderkindedu.uz/verify-certificate/{cert.Id}</x>", html);
    }
